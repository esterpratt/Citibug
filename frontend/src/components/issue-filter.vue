<template>
  ...
  <section class="issue-filter">
    ...
    <div :class="{open: isOpen, close: !isOpen}">
      ...
      >
        <el-option v-for="item in issueCategories" :key="item" :value="item"></el-option>
      </el-select>
      <el-radio-group class="status-filter" v-model="filter.byStatus" @change="setFilter">
      ...
      <label class="issue-sort">
        ...
        <el-select v-model="filter.sortBy" @change="setFilter">
          <el-option v-for="item in sortOptions" :key="item" :value="item"></el-option>
        </el-select>
      </label>
    </div>
  </section>
